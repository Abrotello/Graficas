...
public class Figuras {
    ...
    public void bresenham(int x1, int y1, int x2, int y2, Color color, BufferedImage imagen) {
    ...
    public void circulo(int xcentro, int ycentro, int radio, double angulo, Color color, BufferedImage imagen) {
        this.imagen = imagen;
        int x = 0, y = 0;
    ...
    public void cuadrado(int x1, int y1, int x2, int y2, Color color, BufferedImage image) {
        ...
        bresenham(x1, y1, x3, y3, color, image);
        bresenham(x1, y1, x4, y4, color, image);
        bresenham(x3, y3, x2, y2, color, image);
        bresenham(x4, y4, x2, y2, color, image);
    }

    public void poligono4Lados(int[] x, int[] y, Color c, BufferedImage bufferedImage) {

        bresenham(x[0], y[0], x[1], y[1], c, bufferedImage);
        bresenham(x[0], y[0], x[2], y[2], c, bufferedImage);
        
        bresenham(x[1], y[1], x[3], y[3], c, bufferedImage);
        bresenham(x[2], y[2], x[3], y[3], c, bufferedImage);
    ...
    public void floodFillCirculo(int xcentro, int ycentro, int radio, BufferedImage bufferedImage, Color color) {
        while(radio != 0) {
            radio--;
            circulo(xcentro, ycentro, radio, 0, color, bufferedImage);
        }
    }

    public void rellenarRectangulo(int x1, int y1, int x2, int y2, Color relleno) {
        y1 += 1;
        x2 -= 1;
        while(y1 < y2) {
            bresenham(x1, y1, x2, y1, relleno, imagen);
            y1++;
        }
    }
